...
export default function Messages() {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => handleLogout(token)}>Logout</button>
      <h2>Mensagens:</h2>
      ...
      <ul>
        {messages.map((message) => {
          return (
            <li key={message._id}>
              <p>{message.text}</p>
              <p>Enviado por: {message.ownerName}</p>
              <p>Enviada em: {message.date}</p>
              <button type="button" onClick={() => handleDeleteMessage(message._id)}>Deletar</button>
            </li>
          )
        })}

        <input placeholder="Digite aqui sua mensagem" value={text} onChange={e => setText(e.target.value)} />
        <button type="button" onClick={() => handleNewMessage()}>Enviar mensagem</button>
      </ul>
    </div>
  )
}